...
int main (int argc, char *argv[])
{
  ...
  timetDateString(oldDateStr, postCsv.logTime, OUTFILE_FORMAT_DSLOGCSV);
  ...
  while(postCsv.logTime < postCsv.endTime) {
    ...
    timetDateString(dateStr, postCsv.logTime, OUTFILE_FORMAT_DSLOGCSV);
    timetTimeString(timeStr, postCsv.logTime, OUTFILE_FORMAT_DSLOGCSV);
	...
	timetDateString(outDateStr, postCsv.logTime, csvOutputList[outputIndex].logging.outputFileFormat);
	timetTimeString(outTimeStr, postCsv.logTime, csvOutputList[outputIndex].logging.outputFileFormat);

	makeCsvOutputString(outputRecord, outputIndex, lastOutputTime[outputIndex], outDateStr, outTimeStr, postCsv.logTime);
...
//---------------------------------------------------------------------------
int timetDateString(char *str, time_t timetValue, int outputFileFormat) 
{
  tm *tmTime;
  ...
  case OUTFILE_FORMAT_SAMOS:
    snprintf (str,32,"%04d%02d%02d", (int) tmTime->tm_year +1900, (int) tmTime->tm_mon + 1, (int) tmTime->tm_mday);
    break;
...
//---------------------------------------------------------------------------
int timetTimeString(char *str, time_t timetValue, int outputFileFormat) 
{
  tm *tmTime;
  ...
  switch(outputFileFormat) {
  case OUTFILE_FORMAT_SAMOS:
    snprintf (str,32,"%02d%02d%02d", tmTime->tm_hour, tmTime->tm_min, tmTime->tm_sec);
    break;
  case OUTFILE_FORMAT_NOAA:
    str[0] = '\0';
    break;
