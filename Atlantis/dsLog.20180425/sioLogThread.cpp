...
sioLogThread (void *thread_num)
{
  ...
  while (1) {
	...
	{			// received a SAS, probably from my subthread
	  char    rawOutputRecord[MAX_LOGGING_STRING_LENGTH];
	  int rawDataLength;
#ifdef DEBUG_SIO
	  ...
	  //--- copy received string to rawOutputRecord BEFORE stripping off terminators ---
	  rawDataLength = snprintf(rawOutputRecord,MAX_LOGGING_STRING_LENGTH,"%s" ,data);
	  rawOutputRecord[rawDataLength] = '\0';   // null terminate just in case
	  ...
	  for(int destinationCount = 0; destinationCount < theLoggingList[myLoggerNumber].nOfDestinations; destinationCount++) {
		...
		case UDP_SOCKET:
		  {
		    ...
		    if(send) {
		      if(1 == theLoggingList[myLoggerNumber].destinations[destinationCount].rawMode) {
			sendto ( theLoggingList[myLoggerNumber].destinations[destinationCount].networkDestination.socket, rawOutputRecord, rawDataLength, 0, (struct sockaddr *) &(theLoggingList[myLoggerNumber].destinations[destinationCount].networkDestination.Addr), sizeof (theLoggingList[myLoggerNumber].destinations[destinationCount].networkDestination));
			destinationStatus.statuses[destinationCount].bytesSent += rawDataLength;
		      }
		      else {
		...
		case DISK:
		  {
		    if(1 == theLoggingList[myLoggerNumber].destinations[destinationCount].rawMode) {
		      logThisNow(&(theLoggingList[myLoggerNumber].destinations[destinationCount]),&(theLoggingList[myLoggerNumber]),rawOutputRecord);
		      destinationStatus.statuses[destinationCount].bytesSent += rawDataLength;
		    }
		    else {
		...
		case SINGLE_DISK:
		  {
		    if(1 == theLoggingList[myLoggerNumber].destinations[destinationCount].rawMode) {
		      logThisNow(&theGlobalLogFile,&theGlobalLog,rawOutputRecord);
		      destinationStatus.statuses[destinationCount].bytesSent += rawDataLength;
		    }
		    else {
