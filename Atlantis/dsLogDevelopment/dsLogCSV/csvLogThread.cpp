...
csvLogThread (void *thread_num)
{
  ...
  while (1) {
	...
	{			// received a SAS, probably from my subthread
	  ...
	  //-------------------  cut --------------------------
	  csvParseData( (char *) data, myLoggerNumber, rov_get_time() );
	  //-------------------  cut --------------------------
	  break;
	}  // end of case SAS:
...
int csvParseData(char *data, int myLoggerNumber, rov_time_t dataRecordTime)
{
  ...
  for(int parseIndex=0; parseIndex < theLoggingList[myLoggerNumber].numberOfCsvParse; ++parseIndex) {
	...
	if(4 == sscanf(instring, "$GPRMC,%2d%2d%2d.%2d", &hour, &minute, &second, &fraction)) {
	  csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].lastRxTime = dataRecordTime;
	...
	if(1 == sscanf(instring, "$WIR%*d,%*[^','],%*[^','],%*[^','],%*[^','],%*[^','],%*[^','],%*[^','],%*[^','],%*[^','],%lf", &tmpval)) {
      ...
      //---------------------------------
    case CPD_TRANS_ARG1:
      instring = strstr((char*)data, "TRANS");
      if(NULL != instring) {
	int tmpval;
	//--- skip date and time
	if(1 == sscanf(instring, "TRANS%*s %*s\t%d", &tmpval)) {
	  csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].lastRxTime = dataRecordTime;
	  sprintf(csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].dataString, "%d", tmpval);
	}
      }
	...
	int tmpval;
	//--- skip date and time
	if(1 == sscanf(instring, "TRANS%*s %*s\t%*d\t%d", &tmpval)) {
	  csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].lastRxTime = dataRecordTime;
	  sprintf(csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].dataString, "%d", tmpval);
	}
      }
	...
	int tmpval;
	//--- skip date and time
	if(1 == sscanf(instring, "TRANS%*s %*s\t%*d\t%*d\t%d", &tmpval)) {
	  csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].lastRxTime = dataRecordTime;
	  sprintf(csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].dataString, "%d", tmpval);
	}
      }
	...
	double tmpval;
	//--- skip date and time
	if(1 == sscanf(instring, "TRANS%*s %*s\t%*d\t%*d\t%*d\t%lf", &tmpval)) {
	  csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].lastRxTime = dataRecordTime;
	  sprintf(csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].dataString, "%.3lf", tmpval);
	}
      }
	...
	int tmpval;
	//--- skip date and time
	if(1 == sscanf(instring, "TRANS%*s %*s\t%*d\t%*d\t%*d\t%*lf\t%d", &tmpval)) {
	  csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].lastRxTime = dataRecordTime;
	  sprintf(csvAbstractData[theLoggingList[myLoggerNumber].csvParse[parseIndex].parseAbstractIndex].dataString, "%d", tmpval);
	}
      }
