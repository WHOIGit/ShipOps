#!/bin/bash
#
# Kill all python feeders for Armstrong website data screens - need to run as root

pkill -f cnav-feeder.py #lat, long, sog, cog from CNAV
pkill -f dps-feeder.py
pkill -f posmv-feeder.py
...
#pkill -f ssv-feeder-4548.py #calculated from sbe45sal and sbe48temp
pkill -f ssv-feeder-valeport.py
pkill -f trans10-feeder.py 
pkill -f trans25-feeder.py

pkill -f knudsen12-feeder.py
#knudsen35-feeder.py & #need to make feeder
pkill -f em712-feeder.py
pkill -f em124-feeder.py
