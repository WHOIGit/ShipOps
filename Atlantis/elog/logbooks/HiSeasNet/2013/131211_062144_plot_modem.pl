...
# only works in 24's.. 
#   /ships/plot_comtech_gnu.pl?span=48
#
##                                 lstolp 20120207
...
my $GNUPLOT = "/usr/bin/gnuplot";
my $PPMTOTIF = "/usr/bin/ppm2tiff";
my $gpfile_1 = "/tmp/cvs/mod_1_$$.cmd";
my $gpfile_2 = "/tmp/cvs/mod_2_$$.cmd";
my $gpfile_3 = "/tmp/cvs/mod_3_$$.cmd";
my $giffile_1 = "mod_1_$$.gif";
my $giffile_2 = "mod_2_$$.gif";
my $giffile_3 = "mod_3_$$.gif";
my $xsize = "1";
my $ysize = ".5";
...
my $tmp_dir = "/var/www/html/timages";
 ...
 print STDERR "GET \$end_datetime_str = $end_datetime_str\n" if ($Debug);
    ...
    for my $file (@file_list){
       ...
       ## create a hash of the header info to figure out which column it is
        ...
        print OUT "$plot_time $ebno $agc $ber\n";
   }
...
##########################################################################
open (GPFILE, "> $gpfile_1");

# Everything after the following line up to the terminating "EOM" line
# defines the contents of the gnuplot command file.
...
plot '$datafile' using 1:2 title "EbNo" with points
EOM
close GPFILE;
...
plot '$datafile' using 1:3 title "AGC" with points
EOM
close GPFILE;


##########################################################################
open (GPFILE, "> $gpfile_3");
...
plot '$datafile' using 1:4 title "BER" with points
EOM
close GPFILE;
...
system ("$GNUPLOT $gpfile_1| $PPMTOGIF   2> /dev/null > "
        ."$httproot/$imagedir/$giffile_1");
system ("$GNUPLOT $gpfile_2| $PPMTOGIF   2> /dev/null > "
        ."$httproot/$imagedir/$giffile_2");
system ("$GNUPLOT $gpfile_3| $PPMTOGIF   2> /dev/null > "
        ."$httproot/$imagedir/$giffile_3");
...
#################make web ######################
 ...
 print <<EOM;
<HTML>
<head> <title> Comtech Modem </title> </head>

<BODY bgcolor=#FFFFFF>
$logo
<center><h4>EbNo </h4> </center>
<h5>Latest EbNo: $ebno dB </h5>
<IMG SRC=/$imagedir/$giffile_1><p>
</br>
<center><h4>AGC</h4> </center>
<h5>Latest ACG: $agc counts </h5>
<IMG SRC=/$imagedir/$giffile_2><p>
</br>
<center><h4>BER </h4> </center>
<h5>Latest BER: $ber dB </h5>
<IMG SRC=/$imagedir/$giffile_3><p>
</br>
