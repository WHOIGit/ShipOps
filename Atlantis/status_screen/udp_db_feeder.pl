...
# drop the * and + from the front of the flr string
                ...
                @strings = split (" ", $mesg);
                $type = $strings[3];
print "SBE48 should be the type, the type is $type  \n" if $debug;
#print "strings4 is $strings[4]  \n" if $debug;
                if (defined $strings[4] && $strings[4] eq "#")
                {
print "strings4 is $strings[4]  \n" if $debug;
                  $field1 = $strings[2];
                  $field2 = $strings[5];
                  #remove the comma from the end of the temp data
		  $field2 = substr($field2,1,(length($field2)-2));
print "SBE48 values are $type $field1 $field2 \n" if $debug;
                } 
             } else {
