...
my $type;
my $udp_type = "";
###pick which surface sound velocity source your want to match
###     SBE45calc = SBE45T & SBE45S used in perl Sound Velocity Calculation module
###     SBE48calc = SBE48T & SBE45S used in perl Sound Velocity Calculation module
###     SBE45raw  = sound velocity as output by thermosalinograph
###  all values are found on udp 55500 from dsLogCSV - ie calculations are done on atdatalog computer
my $ssv_pick = "SBE48calc";
...
# pick out the SSV value desired 
            elsif ($udp_type eq "ssv") {
                @strings = split (" ", $mesg);
                $type = "SSV";
                my $comp_type = $strings[3];
print STDERR "\$comp_type = $comp_type\n" if ($debug);
print STDERR "\$ssv_pick = $ssv_pick\n" if ($debug);


#find the one you want
                if ($ssv_pick eq $comp_type) {

print STDERR "They match - yay\n" if ($debug);
                    $field1 = $strings[2];
                    $field2 = $strings[4];
                    }
             }
