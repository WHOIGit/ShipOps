...
int makeCsvOutputString(char *outputRecord, int myLoggerNumber, int lastCsvMsgTime, char *dateString, char *timeString, time_t timeNow)
{
  ...
  if(csvOutputList[myLoggerNumber].numberOfOutputFields > 0) {
	...
	switch(csvOutputList[myLoggerNumber].outfieldType[outputIndex]) 
	  ...
	  default:
	    // post-testing-code
	    //printf("outputIndex=%d  lastRxTime=%lf lastCsvMsgTime=%d difference=%.3lf\n", outputIndex, csvAbstractData[csvOutputList[myLoggerNumber].outfieldAbstractIndex[outputIndex]].lastRxTime, lastCsvMsgTime, csvAbstractData[csvOutputList[myLoggerNumber].outfieldAbstractIndex[outputIndex]].lastRxTime - (double) lastCsvMsgTime);
