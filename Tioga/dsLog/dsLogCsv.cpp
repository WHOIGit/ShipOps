         ...
         if( UDP_ASCII == theLoggingList[nOfLoggers].sourceType)
            {
               ...
               make_thread_table_entry (LOG_THREAD_OFFSET + nOfLoggers, threadName, csvLogThread, (void *) (LOG_THREAD_OFFSET + nOfLoggers),nOfLoggers, NULL_EXTRA_ARG_VALUE, NULL_EXTRA_ARG_VALUE,NULL_EXTRA_ARG_VALUE);
  ...
  *nOfLoggers = 0;
  ...
  for(int inputNumber = 0; inputNumber < MAX_N_OF_INPUTS; inputNumber++) {
    ...
    { //----- section to read ini file info for parsing csv data
      ...
      thisDescriptor.numberOfCsvParse = rov_read_int(ini_fd, inputField,fieldName,0);
      printf("***** numberOfCsvParse is %d *****\n", thisDescriptor.numberOfCsvParse);

      if(thisDescriptor.numberOfCsvParse > 0) {
	for(int promptNumber = 0; promptNumber < thisDescriptor.numberOfCsvParse; promptNumber++) {
	  //  int csvParseIndex;  // indicates what field this is
	  snprintf(fieldName,64,"PARSE_INDEX_%d", promptNumber+1);
	  thisDescriptor.csvParse[promptNumber].csvParseIndex = rov_read_int(ini_fd, inputField, fieldName, 1);
	  printf("\n***** parseIndex[%d] is %d *****\n", promptNumber, thisDescriptor.promptEnable[promptNumber]);

	  //  int valueType;
	  snprintf(fieldName,64,"PARSE_VALUE_TYPE_%d", promptNumber+1);
	  thisDescriptor.csvParse[promptNumber].valueType = rov_read_int(ini_fd, inputField, fieldName, 0);
	  // make sure there is a valid value
	  switch(thisDescriptor.csvParse[promptNumber].valueType) {
	  case PARSE_VALUE_TYPE_INT:
	  case PARSE_VALUE_TYPE_DOUBLE:
	  case PARSE_VALUE_TYPE_STRING:
	    break;
	  default:
	    thisDescriptor.csvParse[promptNumber].valueType = PARSE_VALUE_TYPE_INT;
	    break;
	  }
	  switch(thisDescriptor.csvParse[promptNumber].valueType) {
	  case PARSE_VALUE_TYPE_INT:
	  ...
	  thisDescriptor.csvParse[promptNumber].intFormatType = rov_read_int(ini_fd, inputField, fieldName, 1);
	  // make sure there is a valid value
	  switch(thisDescriptor.csvParse[promptNumber].intFormatType) {
	  ...
	  case PARSE_INT_FORMAT_TYPE_OCTAL_NONE:
	    break;
	  default:
	    thisDescriptor.csvParse[promptNumber].intFormatType = PARSE_INT_FORMAT_TYPE_DECIMAL;
	    break;
	  }
	  switch(thisDescriptor.csvParse[promptNumber].intFormatType) {
	  case PARSE_INT_FORMAT_TYPE_DECIMAL:
	  ...
	  //  int parameterIndex;
	  snprintf(fieldName,64,"PARAMETER_INDEX_%d", promptNumber+1);
	  thisDescriptor.csvParse[promptNumber].parameterIndex = rov_read_int(ini_fd, inputField, fieldName, 1);
	  printf("***** parameterIndex[%d] is %d *****\n", promptNumber, thisDescriptor.csvParse[promptNumber].parameterIndex);

	  //  char *prefixString;
	  snprintf(fieldName,64,"PREFIX_STRING_%d", promptNumber+1);
	  scratchString = rov_read_string(ini_fd, inputField, fieldName, "INVALID_PREFIX");
	  thisDescriptor.csvParse[promptNumber].prefixString = strdup(scratchString);
	  printf("***** prefixString[%d] is %s *****\n", promptNumber, thisDescriptor.csvParse[promptNumber].prefixString);
	  ...
	  thisDescriptor.csvParse[promptNumber].delimiterString = strdup(scratchString);
	  printf("***** delimiterString[%d] is %s *****\n", promptNumber, thisDescriptor.csvParse[promptNumber].delimiterString);

	  //  char *rawScanfString;
	  snprintf(fieldName,64,"RAW_SCANF_STRING_%d", promptNumber+1);
	  scratchString = rov_read_string(ini_fd, inputField, fieldName, "XXXX");
	  thisDescriptor.csvParse[promptNumber].rawScanfString = strdup(scratchString);
	  printf("***** rawScanfString[%d] is %s *****\n", promptNumber, thisDescriptor.csvParse[promptNumber].rawScanfString);
	}
      }  // end of if(thisDescriptor.numberOfCsvParse > 0)
    ...
    printf("**** **** thisDescriptor.nOfDestinations is %d **** ****\n", thisDescriptor.nOfDestinations);
