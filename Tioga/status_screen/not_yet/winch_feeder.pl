...
while (1) {
	...
	print STDERR "Input message: $msg\n" if $debug;
        ...
        $host = $winch_udp->peerhost();
        print STDERR "Message from: $host\n" if $debug;

        if ($host eq "192.168.11.240") {
		print STDERR "Message from correct host: $host\n" if $debug;
...
#
			if ($winch_input[2]  =~ /01/) {
				$winch_input[2] = "Port Hydro";
                                print STDERR "winch is $winch_input[2]\n" if $debug;
			}
			elsif ($winch_input[2] =~ /03/) {
				$winch_input[2] = "Stbd Hydro";
			}
			elsif ($winch_input[2] =~ /05/) {
				$winch_input[2] = "Trawl";
			}
