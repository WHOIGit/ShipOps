...
#  version 20130129
#
#  20130923        lstop took out first $sock statement, before loop
#  20140410        lstolp added line to gyro to take only $GPHDT,26.4,T*05
#
...
# drop the * and + from the front of the flr string
                ...
                @strings = split (" ", $mesg);
                $type = $strings[3];
                 next if (!defined ($strings[4])); 
                 if ($strings[4] eq "#")
                 {
                  $field1 = $strings[2];
                  $field2 = $strings[5];
                  print "\$strings[5] = $strings[5]" if ($debug);
                  my $chr = chop($field2);
                }
