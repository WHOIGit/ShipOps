...
#        sleep (.1);
        ...
        my $dep = shift @data;
          ...
          if ($dep =~ /\$PKEL99/)
           ...
           print "found \$PKEL99\n" if $debug;
           print "\@data = @data\n" if $debug;
           my @pkel = qw /Depth200 200valid draft200 Depth33 33valid draft33 sspeed position/;
   ...
   foreach $key (@keys)
    {
      if ($key eq "Depth200")
       { 
              ...
              VALUES ('$key', '$field1', '$depth{Depth200}')");
       }
      if ($key eq "Depth33")
       { 
              $dbh->do( "INSERT OR REPLACE INTO latest_data (header, udp_time, data)
              VALUES ('$key', '$field1', '$depth{Depth33}')");
       }
