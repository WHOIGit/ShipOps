//---------------------------------------------------------------------------
//
// This code was provided by Jonathan Beaudoin of UNOLS
// 
// His email states:
...
// 
// This routine expects Pressure in bars, so it seems the proper value is 0.5 Bars.
// 
// 
//---------------------------------------------------------------------------
// Revision history:
...
//
//-----------------------------
double ssv_ChenMillero (double sal, double temp, double press)
{
  ...
  A =
    (a00 + a01 * temp + a02 * pow(temp, 2)   + a03 * pow(temp, 3) + a04 * pow(temp, 4)) 
    + (a10 + a11 * temp + a12 * pow(temp, 2) + a13 * pow(temp, 3) + a14 * pow(temp, 4)) * press 
    + (a20 + a21 * temp + a22 * pow(temp, 2) + a23 * pow(temp, 3)) * pow(press, 2) 
    + (a30 + a31 * temp + a32 * pow(temp, 2)) * pow(press, 3);

  Cw =
    (c00 + c01 * temp + c02 * pow(temp, 2)   + c03 * pow(temp, 3) + c04 * pow(temp, 4) + c05 * pow(temp, 5))
    + (c10 + c11 * temp + c12 * pow(temp, 2) + c13 * pow(temp, 3) + c14 * pow(temp, 4)) * press 
    + (c20 + c21 * temp + c22 * pow(temp, 2) + c23 * pow(temp, 3) + c24 * pow(temp, 4)) * pow(press, 2) 
    + (c30 + c31 * temp + c32 * pow(temp, 2)) * pow(press, 3);
