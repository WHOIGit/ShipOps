...
#                  
#  20161010        lws
#                  - changed database to be written to memory, using tmpfs filesystem
#                  - added a sleep 1, since this comes in so fast
#
#
#  version 20130129
#
...
while (1) {
        ...
        $field1 = $strings[1];
        $wave_height = $strings[5];
        $wave_dir = $strings[4];
        $wave_per = $strings[3];
        $current_sp = $strings[6];
        $current_dir = $strings[7];
...
my $trigger_result = $dbh->do("CREATE TRIGGER insert_data_timeEnter AFTER INSERT ON latest_data
        ...
        END");
              ...
              VALUES ('WAVE_HEIGHT', '$field1', '$wave_height')");
              $dbh->do( "INSERT OR REPLACE INTO latest_data (header, udp_time, data)
              VALUES ('WAVE_DIR', '$field1', '$wave_dir')");
              $dbh->do( "INSERT OR REPLACE INTO latest_data (header, udp_time, data)
              VALUES ('WAVE_PER', '$field1', '$wave_per')"); 
              $dbh->do( "INSERT OR REPLACE INTO latest_data (header, udp_time, data)
              VALUES ('CUR_SP', '$field1', '$current_sp')");
              $dbh->do( "INSERT OR REPLACE INTO latest_data (header, udp_time, data)
              VALUES ('CUR_DIR', '$field1', '$current_dir')");
