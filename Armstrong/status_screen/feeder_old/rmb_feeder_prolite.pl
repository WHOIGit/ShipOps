...
#
#   20181122	   cts
#		   -added output to read on prolite 
#
#  version 20160806
#
...
while (1) {
        ...
        my $outsock = IO::Socket::INET->new(Proto=> 'udp', PeerPort => $outport, PeerAddr => $outaddr);
	$sock->recv($mesg, 180);
...
#           print "\$strings[4] = $strings[4]\n" if $debug;
       ...
       print "\$spd is $spd\n" if $debug;
       	...
       	$distance = $rmb{DTWP};
	my $EOL = "\015\012";
	my $outmsg = '<ID01><PA><CC><FD>Dst: '.$distance.' nm';
	$outsock->send('<ID01>'.$EOL);
	
	$outsock->send($outmsg.$EOL);
#	$outsock->send("<ID01><RPA>".$EOL); 
	sleep(1);
	if ($spd<= 0.25) {
		$rmb{TTWP} = 0;    
